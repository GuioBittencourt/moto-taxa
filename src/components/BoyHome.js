...
export default function BoyHome({ perfil, onLogout }) {
  ...
  async function carregarEntregas(turnoId) {
    const { data } = await supabase.from('entregas').select('*').eq('turno_id', turnoId).order('created_at', { ascending: false })
    setEntregas(data || [])
  }
  ...
  async function abrirTurno() {
    ...
    const { data } = await supabase.from('turnos').insert({
      ...
      inicio: new Date().toISOString(), taxa_fixa_turno: estabAtivo.taxa_fixa_turno || 0, status: 'aberto'
    }).select().single()
  ...
  if (tela === 'add-estab') return (
    <CadastroEstabelecimento
      userId={perfil.id}
      onSalvo={(e) => { setEstabelecimentos(p => [...p, e]); setEstabAtivo(e); setTela('home') }}
  ...
  if (tela === 'relatorio') return (
    <Relatorio perfil={perfil} turno={turnoAtivo} estabelecimento={estabAtivo} entregas={entregas} onVoltar={() => setTela('home')} />
  )
      ...
      <div className="card">
        ...
        {estabelecimentos.length === 0 ? (
          <>
            ...
            <button className="btn btn-primary" onClick={() => setTela('add-estab')}>+ Cadastrar estabelecimento</button>
          </>
        ) : (
          <>
            ...
            {estabAtivo && (
              <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 8 }}>
                ...
                {estabAtivo.taxa_fixa_turno > 0 && <span style={{ color: 'var(--yellow)', marginLeft: 6 }}>+R${estabAtivo.taxa_fixa_turno}/turno</span>}
              </div>
            )}
            ...
            <button className="btn btn-outline" style={{ marginTop: 4, fontSize: 12 }} onClick={() => setTela('add-estab')}>+ Outro estabelecimento</button>
          </>
        )}
      </div>
...
function Relatorio({ perfil, turno, estabelecimento, entregas, onVoltar }) {
    ...
    <div style={{ padding: '0 1rem' }}>
      ...
      <div className="card">
        <div style={{ textAlign: 'center', padding: '1rem 0' }}>
          <img src="/logo-escudo.png" alt="" style={{ width: 64, height: 64, objectFit: 'contain', marginBottom: 10 }} />
          <div style={{ fontFamily: 'Barlow Condensed', fontSize: 18, fontWeight: 700 }}>{estabelecimento?.nome}</div>
          <div className="muted">{new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}</div>
        </div>

        <div className="grid2" style={{ marginTop: 8 }}>
          <div className="metric"><div className="metric-val">R${total.toFixed(2)}</div><div className="metric-lbl">Entregas</div></div>
